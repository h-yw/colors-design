...
// --- Strict Token Types ---
export type BrandTokenKeys = `brand.${
  | 'primary'
  | 'primary-hover'
  | 'primary-bg'}`;
export type TextTokenKeys = `text.${
  | 'primary'
  | 'secondary'
  | 'placeholder'
  | 'inverse'
...
export type SemanticTokenKeys = `${'success' | 'warning' | 'error' | 'info'}.${
  | 'text'
  | 'bg'
  | 'border'
  | 'solid.bg'
...
export type SystemTokens = Partial<Record<SystemTokenKey, string>> &
...
export class TraditionalColorSystem {
  ...
  generateTonalPalette(
    inputColor: string | Oklch,
    gamutOverride?: 'srgb' | 'p3'
  ): Palette {
    const source =
      typeof inputColor === 'string'
        ? (toOklch(inputColor) as Oklch)
        : inputColor;
    const tones: Palette = {};
    ...
    levels.forEach((level) => {
      ...
      if (targetGamut === 'p3' && !isDisplayable(newColor, 'srgb')) {
        ...
        if (p3) {
          tones[level] = `color(display-p3 ${formatPrecise(
            p3.r
          )} ${formatPrecise(p3.g)} ${formatPrecise(p3.b)})`;
        } else {
   ...
   */
  generateHarmonies(inputColorName: string): {
  ...
  generatePalette(
    ...
    gamutOverride?: 'srgb' | 'p3',
    overrides?: { secondary?: string; tertiary?: string }
  ): GeneratedSystem {
    const theme =
      this.library.find((c) => c.name === themeName) || this.library[0];
    ...
    const neutralSource = {
      ...sourceOklch,
      c: Math.min(sourceOklch.c * 0.1, 0.03),
    };
    ...
    const neutralVariantSource = {
      ...sourceOklch,
      c: Math.min(sourceOklch.c * 0.2, 0.06),
    };
    const neutralVariantPalette = this.generateTonalPalette(
      neutralVariantSource,
      gamutOverride
    );
    ...
    const secondaryPalette = this.generateTonalPalette(
      secondarySource,
      gamutOverride
    );
    ...
    const tertiaryPalette = this.generateTonalPalette(
      tertiarySource,
      gamutOverride
    );
    ...
    const fourthSource = {
      ...sourceOklch,
      h: ((sourceOklch.h || 0) + 180) % 360,
    };
    const fourthPalette = this.generateTonalPalette(
      fourthSource,
      gamutOverride
    );

    // Emphasize: High Vibrancy
    const emphasizeSource = {
      ...sourceOklch,
      c: Math.min(0.4, sourceOklch.c * 1.5),
    }; // Boost Chroma
    const emphasizePalette = this.generateTonalPalette(
      emphasizeSource,
      gamutOverride
    );
  ...
  private pickContrastColor(bg: string, light: string, dark: string): string {
    const contrastLight = Math.abs(calcAPCA(light, bg));
    const contrastDark = Math.abs(calcAPCA(dark, bg));
    // Prioritize the one with higher perceptual contrast magnitude
    return contrastLight >= contrastDark ? light : dark;
  }

  private getBrandTokens(
    primitives: Primitives,
    isDark: boolean
  ): SystemTokens {
  ...
  private getTextTokens(primitives: Primitives, isDark: boolean): SystemTokens {
    ...
    // Smart Contrast Text
    sys['text.on-brand'] = this.pickContrastColor(
      isDark ? primitives.brand[80] : primitives.brand[40],
      white,
      black
    );
    sys['text.on-error'] = this.pickContrastColor(
      isDark ? primitives.error[80] : primitives.error[40],
      white,
      black
    );
    sys['text.on-success'] = this.pickContrastColor(
      isDark ? primitives.success[80] : primitives.success[40],
      white,
      black
    );
    sys['text.on-warning'] = this.pickContrastColor(
      isDark ? primitives.warning[80] : primitives.warning[90],
      white,
      black
    );
  ...
  private getActionTokens(
    ...
    const mapAction = (level: string, palette: Palette) => {
      if (!isDark) {
        ...
        sys[`text.on-${level}`] = this.pickContrastColor(
          palette[40],
          white,
          black
        );
  ...
  private getEffectTokens(isDark: boolean): SystemTokens {
    ...
    if (!isDark) {
      ...
      sys['effect.shadow.md'] =
        '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)';
      sys['effect.shadow.lg'] =
